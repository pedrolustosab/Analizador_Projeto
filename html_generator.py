...
def get_html_template(d):
    ...
    if d['has_finance']:
        ...
        eva_content = f"""
            <div class='eva-grid'>
                <div class='eva-card'><span>Budget Original (BAC)</span><b>{fmt(d['bac'])}</b><small>Total Aprovado</small></div>
                ...
                <div class='eva-card'><span>Custo Incorrido (AC)</span><b>{fmt(d['ac'])}</b><small>Gasto Real</small></div>
                <div class='eva-card'><span>Var. Cronograma (SV)</span><b class='{'pos' if d['sv']>=0 else 'neg'}'>{fmt(d['sv'])}</b><small>EV - PV</small></div>
                <div class='eva-card'><span>Var. Custo (CV)</span><b class='{'pos' if d['cv']>=0 else 'neg'}'>{fmt(d['cv'])}</b><small>EV - AC</small></div>
                <div class='eva-card'><span>Desempenho Prazo (SPI)</span><b class='{'pos' if d['spi']>=1 else 'neg'}'>{d['spi']:.2f}</b><small>> 1.0 é positivo</small></div>
                <div class='eva-card'><span>Desempenho Custo (CPI)</span><b class='{'pos' if d['cpi']>=1 else 'neg'}'>{d['cpi']:.2f}</b><small>> 1.0 é positivo</small></div>
                <div class='eva-card'><span>Estimativa Final (EAC)</span><b>{fmt(d['eac'])}</b><small>Custo projetado</small></div>
                <div class='eva-card'><span>Desvio Projetado (VAC)</span><b class='{'pos' if d['vac']>=0 else 'neg'}'>{fmt(d['vac'])}</b><small>BAC - EAC</small></div>
            </div>
